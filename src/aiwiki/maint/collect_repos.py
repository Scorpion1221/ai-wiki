...
def commit_rows(
    object_repo: Path, previous: str | None, current: str, limit: int, *pathspec: str
) -> tuple[list[dict[str, str]], int]:
    revision = f"{previous}..{current}" if previous else current
    count = int(run_git("rev-list", "--count", revision, "--", *pathspec, cwd=object_repo) or "0")
    output = run_git(
        "log",
        f"--max-count={limit}",
...
# Repos collector (design §4.3): a scan report becomes planner candidates with frozen evidence
# and the per-repository ``repos`` cursor. Write that cursor only after the items are enqueued.

COLLECTOR = "repos"
DOC_SUFFIXES = frozenset({".md", ".markdown", ".mdx", ".txt", ".rst"})


def cursor_from_report(report: dict[str, Any]) -> dict[str, dict[str, Any]]:
    """The ``repos`` cursor value ``{<canonical remote>: {branch, sha, stale_since, error}}``."""
    cursor = {
        canonical_remote(row["remote_url"]): {
            "branch": row.get("branch"),
            "sha": row["sha"],
            "stale_since": row.get("stale_since"),
            "error": row.get("last_error"),
        }
        for row in report["checkpoint_candidate"]["repos"].values()
    }
    return dict(sorted(cursor.items()))


def checkpoint_from_cursor(cursor: dict[str, dict[str, Any]]) -> dict[str, Any]:
    """A ``--checkpoint-json`` document that makes a scan diff against a non-empty ``repos`` cursor."""
    repos: dict[str, dict[str, Any]] = {}
    for identity, row in cursor.items():
        entry = {
            "name": identity.rsplit("/", 1)[-1],
            "remote_url": identity if identity.startswith("file://") else f"https://{identity}",
            "branch": row["branch"],
            "sha": row["sha"],
        }
        if row.get("stale_since"):
            entry["stale_since"] = row["stale_since"]
        if row.get("error"):
            entry["last_error"] = row["error"]
        repos[repo_id(identity)] = entry
    return {"version": 4, "repos": repos}


def collect(
    report: dict[str, Any], *, previous: dict[str, dict[str, Any]] | None = None,
    exclude_remotes: Iterable[str] = (), prefixes: Iterable[str] = planner.TOPIC_PREFIXES,
) -> dict[str, Any]:
    """Planner candidates for a scan report, plus the ``repos`` cursor to write once they are enqueued.

    A ``changed`` repository yields one candidate per topic: a task root (``<prefix>/<entry>``
    below a topic prefix) or else a top-level directory. Its change list is re-read in full
    from the row's object repository, so the report's path truncation never hides a topic.
    Noise paths are only counted; each candidate freezes a change manifest named after its
    range and the redacted, clipped bytes of its changed text files within the planner's
    limits, each named after its path so a same-topic merge replaces only an older snapshot
    of that path. A ``new`` or ``rebaselined`` repository yields a single
    ``rebaseline:<remote>`` summary instead of a diff across lineages. ``exclude_remotes``
    (the bundle's own repository) yield nothing. A repository whose evidence cannot be read
    keeps its previous cursor row, marked stale since the ``stale_since`` of ``previous`` (the
    ``repos`` cursor the scan diffed against) or now.
    """
    excluded = {canonical_remote(url) for url in exclude_remotes}
    cursor = cursor_from_report(report)
    candidates: list[dict[str, Any]] = []
    failed: list[dict[str, str]] = []
    counts: Counter[str] = Counter({"changed": 0, "rebaselined": 0, "excluded": 0})
    noise: Counter[str] = Counter()
    for row in report["repos"]:
        if row.get("state") not in ("changed", "new", "rebaselined"):
            continue
        identity = canonical_remote(row["remote_url"])
        if identity in excluded:
            counts["excluded"] += 1
        elif row["state"] != "changed":
            candidates.append(rebaseline_candidate(row, identity))
            counts["rebaselined"] += 1
        else:
            try:
                found, found_noise = topic_candidates(row, identity, tuple(prefixes))
            except (OSError, ScanError, ValueError) as exc:
                error = redact(str(exc))
                failed.append({"remote_url": row["remote_url"], "error": error})
                stale_since = (previous or {}).get(identity, {}).get("stale_since") or report["generated_at"]
                cursor[identity] = {"branch": row["previous_branch"], "sha": row["previous_sha"],
                                    "stale_since": stale_since, "error": error[:500]}
                continue
            candidates += found
            noise += found_noise
            counts["changed"] += 1
    ...
    return {"candidates": candidates, "cursor": cursor, "failed": failed,
            "counts": {**dict(sorted(counts.items())), "noise": dict(sorted(noise.items()))}}


def read_blob(object_repo: Path, blob: str) -> bytes:
    try:
        result = subprocess.run(["git", "cat-file", "blob", blob], cwd=object_repo, capture_output=True, check=False,
                                stdin=subprocess.DEVNULL, env=GIT_ENV, timeout=GIT_TIMEOUT_S)
    except subprocess.TimeoutExpired as exc:
        raise ScanError(f"git cat-file timed out after {GIT_TIMEOUT_S}s") from exc
    if result.returncode:
        raise ScanError(f"git cat-file blob {blob} exited {result.returncode}")
    return result.stdout


def tree_blobs(object_repo: Path, commit: str) -> dict[str, str]:
    """path -> blob id for every regular file of ``commit`` (no symlinks or submodules)."""
    blobs = {}
    for entry in run_git("ls-tree", "-r", "-z", commit, cwd=object_repo, strip=False).split("\0"):
...
def evidence_rank(change: dict[str, str]) -> tuple[int, str]:
    """Freeze task-root status docs first, then other docs, then everything else; by path within."""
    path = PurePosixPath(change["path"])
    doc = 0 if path.stem.lower() in planner.TASK_DOCS else 1 if path.suffix.lower() in DOC_SUFFIXES else 2
    return doc, change["path"]


def topic_candidates(
    row: dict[str, Any], identity: str, prefixes: tuple[str, ...]
) -> tuple[list[dict[str, Any]], Counter[str]]:
    object_repo, base, head = Path(row["object_repo"]), row["previous_sha"], row["current_sha"]
    topics: dict[str, list[dict[str, str]]] = {}
    for change in changed_paths(object_repo, base, head):
        topics.setdefault(planner.topic_path(change["path"], prefixes), []).append(change)
    blobs = tree_blobs(object_repo, head)
    candidates: list[dict[str, Any]] = []
    total_noise: Counter[str] = Counter()
    for topic, changes in sorted(topics.items()):
        kinds = [planner.noise(change["path"]) for change in changes]
        noise = Counter(kind for kind in kinds if kind)
        total_noise += noise
        kept = [change for change, kind in zip(changes, kinds, strict=True) if not kind]
        if kept:
            candidates.append(topic_candidate(row, identity, topic, kept, noise, blobs))
    return candidates, total_noise


def topic_candidate(
    row: dict[str, Any], identity: str, topic: str, kept: list[dict[str, str]], noise: Counter[str],
    blobs: dict[str, str],
) -> dict[str, Any]:
    object_repo, base, head = Path(row["object_repo"]), row["previous_sha"], row["current_sha"]
    pathspec = ":(glob)*" if topic == "." else f":(literal){topic}"
    commits, commit_count = commit_rows(object_repo, base, head, DEFAULT_MAX_COMMITS, pathspec)
    remote = {"remote": row["remote_url"], "commit": head}
    lines = [f"# {row['name']} · {topic}", "", f"- remote: {row['remote_url']}", f"- branch: {row['branch']}",
             f"- range: {base}..{head}", f"- commits: {commit_count}"]
    if noise:
        lines.append("- noise (counted, not frozen): " + ", ".join(f"{k} {v}" for k, v in sorted(noise.items())))
    lines += ["", "## Commits", "", *(f"- {c['sha'][:12]} {c['committed_at']} {c['subject']}" for c in commits)]
    if commit_count > len(commits):
        lines.append(f"- … {commit_count - len(commits)} older commits")
    lines += ["", "## Changed paths", ""]
    lines += [f"- {c['status']} {c['path']}" + (f" (from {c['old_path']})" if "old_path" in c else "") for c in kept]
    manifest = "\n".join(lines) + "\n"
    files = [planner.evidence_file(f"changes-{base[:7]}-{head[:7]}.md", manifest,
                                   {"kind": "git-changes", **remote, "base": base})]
    budget, skipped = planner.ITEM_TEXT_LIMIT - files[0]["bytes"], 0
    for change in sorted(kept, key=evidence_rank):
        blob = blobs.get(change["path"])
        ...
        if file is None or file["bytes"] > budget or len(files) >= planner.ITEM_FILE_LIMIT:
            skipped += 1
            continue
        files.append(file)
        budget -= file["bytes"]
    shown = [change["path"] if topic == "." else change["path"].removeprefix(f"{topic}/") for change in kept]
    brief = f"{row['name']} {topic}: {commit_count} commits since {base[:7]}; changed {', '.join(shown)}"
    if skipped:
        brief += f"; {skipped} not frozen"
    if noise:
        brief += f"; noise {sum(noise.values())}"
    return {
        "collector": COLLECTOR,
        "topic_key": f"repo:{identity}#{topic}",
        "origin": {"kind": "repo", **remote, "branch": row["branch"], "base": base},
        "brief": brief,
        "files": files,
        "signals": {"paths": [change["path"] for change in kept]},
    }


def snapshot_name(path: str) -> str:
    """``<path hash>-<basename>`` within the queue's name rule: ``[A-Za-z0-9._-]``, at most 128."""
    name = re.sub(r"[^A-Za-z0-9._-]", "_", PurePosixPath(path).name)
    return f"{hashlib.sha256(path.encode()).hexdigest()[:8]}-{name}"[:128]


def rebaseline_candidate(row: dict[str, Any], identity: str) -> dict[str, Any]:
    """One summary item for a new or rebaselined repository; the Maintainer splits or skips it."""
    reason = row.get("rebaseline_reason") or row["state"]
    base = row.get("merge_base")
    lines = [
        f"# {row['name']} · baseline ({reason})", "",
        f"- remote: {row['remote_url']}",
        f"- branch: {row['branch']} at {row['current_sha']}",
        f"- previous: {row.get('previous_branch')} at {row.get('previous_sha')}",
        f"- merge base: {base}",
        f"- changed paths: {row['change_count']}; commits: {row['commit_count']}",
        "",
        "Not an incremental diff: review this baseline, then split it into topics or skip it.",
        "", "## Commits", "",
        *(f"- {c['sha'][:12]} {c['committed_at']} {c['subject']}" for c in row["commits"]),
        "", "## Path groups", "",
        *(f"- {group}: {count}" for group, count in row["path_groups"].items()),
        "", "## Task roots and prefixes", "",
        *(f"- {group}: {count}" for group, count in row["priority_groups"].items()),
    ]
    origin = {"remote": row["remote_url"], "commit": row["current_sha"], "base": base}
    return {
        "collector": COLLECTOR,
        "topic_key": f"rebaseline:{identity}",
        "origin": {"kind": "repo", **origin, "branch": row["branch"], "reason": reason},
        "brief": (f"{row['name']}: {reason}; {row['change_count']} paths in {len(row['path_groups'])} top-level "
                  f"groups, {row['commit_count']} commits; review the baseline"),
        "files": [planner.evidence_file("baseline.md", "\n".join(lines) + "\n", {"kind": "git-baseline", **origin})],
        "signals": {},
    }
